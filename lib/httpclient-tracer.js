...
module.exports = app => {
  app.httpclient.on('request', req => {
    ...
    app.logger.info('[httpclient] [%s] %s %s , \x1b[33m [[start]] \x1b[0m, req_args: %s',
  ...
  app.httpclient.on('response', result=> {
    ...
    // result.ctx.body 就是 undefined
    app.logger.info('[httpclient] [%s] %s %s \x1b[33m [[end]] \x1b[0m, status: %s, use: %s, response: %s',
      req.ctx.traceId,
      req.args.method,
      `${req.options.host}${req.options.path}`,
      res.status,
      Date.now() - req.starttime,
      JSON.stringify(res.data || 'whathef').substring(0,300)
    );
    /**
